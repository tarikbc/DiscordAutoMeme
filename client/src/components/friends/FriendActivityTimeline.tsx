...
const FriendActivityTimeline = ({
    ...
    if (events.length === 0 && !loading && friendId && !mockDataGeneratedRef.current) {
      ...
      for (let i = 0; i < dayCount; i++) {
        ...
        for (let j = 0; j < eventsPerDay; j++) {
          ...
          const eventTypes: ActivityEvent['type'][] = ['GAME', 'MUSIC', 'STREAMING', 'WATCHING', 'CONTENT_SENT', 'CONTENT_RECEIVED'];
          ...
          switch (type) {
            case 'GAME':
              title = ['Minecraft', 'Fortnite', 'League of Legends', 'Valorant', 'Among Us'][Math.floor(Math.random() * 5)];
          ...
          mockEvents.push({
            ...
            endTime,
            duration,
            contentId: type === 'CONTENT_SENT' || type === 'CONTENT_RECEIVED' ? `content-${Math.floor(Math.random() * 100)}` : undefined
          });
        }
      }
  ...
  const getEventIcon = (type: ActivityEvent['type']) => {
    switch (type) {
      case 'GAME':
        return (
          <svg className="h-5 w-5 text-purple-500 dark:text-purple-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"></path>
          </svg>
        );
      case 'MUSIC':
        return (
          <svg className="h-5 w-5 text-green-500 dark:text-green-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z"></path>
          </svg>
        );
      case 'STREAMING':
        return (
          <svg className="h-5 w-5 text-red-500 dark:text-red-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      case 'WATCHING':
        return (
          <svg className="h-5 w-5 text-blue-500 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zM14.553 7.106A1 1 0 0014 8v4a1 1 0 00.553.894l2 1A1 1 0 0018 13V7a1 1 0 00-1.447-.894l-2 1z"></path>
          </svg>
        );
      case 'CONTENT_SENT':
        return (
          <svg className="h-5 w-5 text-indigo-500 dark:text-indigo-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"></path>
          </svg>
        );
      case 'CONTENT_RECEIVED':
        return (
          <svg className="h-5 w-5 text-pink-500 dark:text-pink-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className={`p-3 rounded-md border-l-4 ${getEventColor(event.type)}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <h4 className="ml-2 text-sm font-medium text-gray-900 dark:text-white">{event.title}</h4>
                        </div>
                        <div className="flex items-center">
                          {event.duration && (
